...
@section('content')
    <div class="container-view">
        ...
        <a href="{{ route('admin.kategori.create') }}" class="btn btn-primary mb-3 btn-aksi shadow-sm">
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('admin.kategori.edit', $item->idkategori) }}" class="btn btn-warning btn-sm btn-aksi me-1" title="Edit Data Kategori">Edit</a>
                                    <form action="{{ route('admin.kategori.destroy', $item->idkategori) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm btn-aksi" title="Hapus Kategori" onclick="return confirm('Yakin ingin menghapus data kategori ini?')">Hapus</button>
                                    </form>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="5" class="text-center text-muted">Tidak ada data Kategori Umum yang ditemukan.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
